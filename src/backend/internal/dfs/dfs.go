    ...
    *outPtr = root
    ...
    for _, rec := range data.Recipes {
        ...
        if totalPaths <= remain {
            leftElem, _  = buildSubtree(recipeMap, ln, leftCount)
            rightElem, _ = buildSubtree(recipeMap, rn, rightCount)
            used = totalPaths
            ...
            leftElem, _ = buildSubtree(recipeMap, ln, leftNeeded)
            ...
            rightElem, _ = buildSubtree(recipeMap, rn, rightNeeded)
...
func DFSSearchParallel(
    ...
    root := &Target{Name: targetName, Tier: rootData.Tier}
    *outTree = root
    ...
    for w := 0; w < numWorkers; w++ {
        go func() {
            ...
            for job := range jobsBuild {
                ...
                leftTree, _  := buildSubtree(recipeData, job.leftName, leftNeed)
                rightTree, _ := buildSubtree(recipeData, job.rightName, rightNeed)
...
func DFSSearchWithUpdates(
    ...
    go func() {
        DFSSearchInternal(recipeData, rootName, maxUniquePaths, outPtr, updates)
        if *outPtr != nil {
            updates <- Update{
                Stage:       "completeDFS",
...
func DFSSearchInternal(
    ...
    root := &Target{Name: targetName, Tier: data.Tier}
    *outPtr = root
    ...
    numWorkers := runtime.NumCPU()
    type countJob struct{ idx int; left, right string }
    ...
    for w := 0; w < numWorkers; w++ {
        go func() {
            ...
            for job := range jobsBuild {
                ...
                leftElem, _ := buildSubtreeInternal(recipeData, job.leftName, leftNeed, updateCh)
                updateCh <- Update{
                    Stage:       "endBuildLeft",
                    ElementName: job.leftName,
                    Tier:        recipeData[job.leftName].Tier,
                ...
                rightElem, _ := buildSubtreeInternal(recipeData, job.rightName, rightNeed, updateCh)
                updateCh <- Update{
                    Stage:       "endBuildRight",
                    ElementName: job.rightName,
                    Tier:        recipeData[job.rightName].Tier,
...
func buildSubtree(
    ...
    var tgt Tree
    DFSSearch(recipeMap, name, maxPaths, &tgt)
    if tgt == nil {
        return nil, 0
    }
	
    e := &Element{Name: tgt.Name, Tier: tgt.Tier}
...
func buildSubtreeInternal(
    ...
    var subtree Tree
    DFSSearchInternal(recipeData, elementName, maxPaths, &subtree, updateCh)
    e := &Element{Name: subtree.Name, Tier: subtree.Tier, Recipes: subtree.Recipes}
