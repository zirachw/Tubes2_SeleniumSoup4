...
interface TreeDataViewerProps {
  elementsData: ElementsData;
  treeData: any;
  loading: boolean;
}

const TreeDataViewer: React.FC<TreeDataViewerProps> = ({
  elementsData,
  treeData,
  loading,
}) => {
  ...
  // helper to give each node a unique ID
  const makeId = () => `n${nodeCountRef.current++}`;

  // Recursively build a "couple" graph out of your JSON
  const buildGraph = useCallback(() => {
    ...
    function walkPair(
      ...
      if (Array.isArray(left.children) && left.children.length) {
        // our shape is { children: [ { left: {...}, right: {...} }, ... ] }
        left.children.forEach((pair: any) =>
          walkPair(pair.left, pair.right, id, true)
        );
      }
      if (Array.isArray(right.children) && right.children.length) {
        right.children.forEach((pair: any) =>
    ...
    if (treeData?.children?.length) {
      ...
      newNodes.push({
        ...
        data: {
          ...
          imageLink: elementsData[treeData.value]?.imageLink || "",
          targetHandles: [],
      ...
      treeData.children.forEach((pair: any) =>
  ...
  }, [treeData, elementsData]);

  // whenever we get (a) data loaded or (b) props change → rebuild graph
  useEffect(() => {
    if (!loading && treeData) {
      buildGraph();
    }
  }, [loading, treeData, buildGraph]);
